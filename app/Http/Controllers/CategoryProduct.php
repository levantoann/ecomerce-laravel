...
class CategoryProduct extends Controller
{
   ...
   public function show_category_home(Request $request, $slug_category_product) { 
      ...
      // 'tbl_category_product.category_id')->where("tbl_category_product.slug_category_product",$slug_category_product)->get();
      
      $category_post = CategoryPostModel::orderBy('cate_post_id','DESC')->get();

      $category_name = DB::table('tbl_category_product')
        ->where('tbl_category_product.slug_category_product', $slug_category_product)->limit(1)
        ->get();
      ...
      return view('pages.category.show_category')->with('cate_product',$cate_product)->with('brand_product',$brand_product)
      ->with('category_by_id',$category_by_id)->with('category_name',$category_name)->with('meta_desc',$meta_desc)
      ->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)
      ->with('slider',$slider)->with('category_post',$category_post)->with('max_price',$max_price)->with('min_price',$min_price)->with('max_price_range',$max_price_range);
   }
  ...
  public function product_tabs(Request $request) {
   ...
   if ($product_count>0) {
      ...
      <div class="tab-pane fade active in" id="tshirt" >
         <div class="col-sm-12">
            <p style="color:red;text-align:center">Hiện chưa có sản phẩm</p>
         </div>
      </div>
   </div>';
   }
